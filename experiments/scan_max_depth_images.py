...
for dname in ['MNIST', 'FashionMNIST', 'SVHN']:
    if dname == 'MNIST':
        ...
        trainset = datasets.KMNIST(root='../data', train=True, download=True, transform=transform_train)
        valset = datasets.KMNIST(root='../data', train=False, download=True, transform=transform_test)
    ...
    for _ in range(4):
        for d in [0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 60, 70, 80, 90, 100]:
            ...
            train_VI_classification(net, name, save_dir, batch_size, nb_epochs, trainset, valset, cuda,
                                ...
                                load_path=None, tags=tags, stop_criteria='test_ELBO')

            exp_version = exp.version
            models_dir = exp.get_data_path(name, exp_version) + '/models'
            filename = models_dir + '/theta_best.dat'
